<?php
/**
 * Template Name: ESL (Board) Page
 *
 * This is the template that displays ESL page with its specific widgets
 *
 * @package ugdsb4
 */
?>
...
<?php $has_left = FALSE; ?>
<?php $has_right = FALSE; ?>
<?php if (is_active_sidebar('sidebar-esl2') || has_nav_menu('left') || has_nav_menu('esl2_menu')) {$has_left = TRUE;} ?>
...
<?php
    ...
    echo '<div class="col-sm-3 col-sm-pull-7 ldiv" id="program">';
        if (has_nav_menu('esl2_menu')){
			bellows( 'main' , array( 'menu' => 114 ) ); //Board ESL menu
        }else{
          get_sidebar('lmenu');//display left menu, sidebar-lmenu.php
        }
   
        get_sidebar('esl2');//display sidebar-left.php
    echo '</div>';
