<?php
/**
 * Template Name: Reopening School Page
 *
 * This is the template that displays Reopening School page with its specific widgets
 *
 * @package ugdsb4
 */
?>
...
<?php
    ...
    if (has_nav_menu('reopening_menu')){
        /*wp_nav_menu(array(  
		...
		bellows( 'main' , array( 'theme_location' => 'reopening_menu' ) ); 
    }else{
       get_sidebar('lmenu');//display left menu, sidebar-lmenu.php
    }
    get_sidebar('reopening');//display sidebar-left.php
    echo '</div>';
    ...
    # Just right sidebar
    elseif (($has_left == FALSE) and ($has_right == TRUE)):
      echo '<div class="col-sm-3" id="school">';
      if (has_nav_menu('reopening_menu')){
		//bellows( 'main' , array( 'menu' => 15 ) ); //school menu id
		bellows( 'main' , array( 'theme_location' => 'reopening_menu' ) );  
      }else{
