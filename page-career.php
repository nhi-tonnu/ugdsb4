 ...
 *
 * This is the template that displays Community page with Career's specific widgets
 *
 * @package ugdsb4
 */
?>
...
<?php $has_left = FALSE; ?>
<?php $has_right = FALSE; ?>
<?php if (is_active_sidebar('sidebar-career') || has_nav_menu('left') || has_nav_menu('career_menu')) {$has_left = TRUE;} ?>
...
<?php
    ...
    # left column
    if (($has_left == TRUE) and ($has_right == TRUE)):
    echo '<div class="col-sm-3 col-sm-pull-7 ldiv" id="board">';
        if (has_nav_menu('career_menu')){
        /*wp_nav_menu(array(  
            'theme_location' => 'career_menu',   // This will be different for you. 
            'container_id' => 'cssmenu', 
            'container_class' => 'ugdsb',
            'walker' => new CSS_Menu_Maker_Walker()
        )); */
		     bellows( 'main' , array( 'theme_location' => 'career_menu' ) );
        }else{
          get_sidebar('lmenu');//display left menu, sidebar-lmenu.php
        }
        get_sidebar('career');//display sidebar-left.php
    echo '</div>';
