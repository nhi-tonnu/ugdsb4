<?php
/**
 * Template Name: ContEd ESL Page
 *
 * This is the template that displays Continuing Education (ESL) page with its specific widgets
 *
 * @package ugdsb4
 */
?>
...
<?php $has_left = FALSE; ?>
<?php $has_right = FALSE; ?>
<?php if (is_active_sidebar('sidebar-esl') || has_nav_menu('left') || has_nav_menu('esl_menu')) {$has_left = TRUE;} ?>
...
<?php
    ...
    echo '<div class="col-sm-3 col-sm-pull-7 ldiv" id="program">';
        if (has_nav_menu('esl_menu')){
        /*wp_nav_menu(array(  
            'theme_location' => 'esl_menu',   // This will be different for you. 
            'container_id' => 'promenu', 
            'container_class' => 'ugdsb',
            'walker' => new CSS_Menu_Maker_Walker()
        )); */
			bellows( 'main' , array( 'menu' => 8 ) );
        }else{
          get_sidebar('lmenu');//display left menu, sidebar-lmenu.php
        }
   
        get_sidebar('esl');//display sidebar-left.php
    echo '</div>';
